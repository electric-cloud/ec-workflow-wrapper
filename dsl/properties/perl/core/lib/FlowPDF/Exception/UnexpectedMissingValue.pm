...
package FlowPDF::Exception::UnexpectedMissingValue;
...
sub render {
    ...
    my $message = 'Got an unexpected missing value.';

    if (ref $params ne 'HASH') {
        $params ||= $message;
        return $params;
    }
    elsif (defined $params->{where} && defined $params->{expected}) {
        $message = sprintf(
            "Value of '%s' could not be missing. '%s' is expected.",
            $params->{where},
            $params->{expected});
    }
    elsif (defined $params->{where}) {
        $message = sprintf("Value of '%s' could not be missing.", $params->{where});
    }
